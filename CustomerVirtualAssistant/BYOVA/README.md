...
Demonstrates the capabilities of Virtual Agent Voice Call Studio Element with graceful call handling, custom exit and re-entry using BYOVA.

## Application Behaviour
* User needs to select Service App from dropdown and then as Scripted / Autonomous and add mandatory desired event name to start event flow in the setting attribute of VirtualAgentVoice 
  * This setting value is mandatory for Webex AI Agent.
  * Add mandatory Config ID and Bot ID.
...
* In the VirtualAgentVoice element, ConnectorType dropdown has been added.
  * Select Service App for BYOVA Apps and then scripted or autonomous dropdown is there, and then add the "Config ID" for service App and "Bot ID" for agent id.
