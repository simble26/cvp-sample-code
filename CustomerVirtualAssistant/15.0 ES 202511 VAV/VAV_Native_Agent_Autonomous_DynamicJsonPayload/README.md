...
Demonstrates the capabilities of Virtual Agent Voice Call Studio Element with graceful call handling, parsing and using dynamic JSON response as event data using  Connector Type as Webex CCAI.
...
- Rest Client is used to make an API call.
- For using dynamic data as Event Data from any API that returns a JSON response, the response must be pre-processed before it is passed to the Event Data field of the VAV node. This pre-processing can be performed using the Set Value element of application as shown in the application.
The processed JSON or the extracted values can then be stored in variables and passed as dynamic Event Data to the VAV node for further processing.
Refer to the Set Value node configuration for an example of parsing the response JSON and extracting the required information.

  ```
    var inputJSON = {Data.Session.response};

    function repairAndEncodeForVav(inputJSON) {
        
        var fixedJSON = String(inputJSON)
                    .replace(/""/g, '","')
                    .replace(/\}{/g, '},{');

        var encodedJSON = String(fixedJSON)
                    .replace(/\\/g, '\\\\')
                    .replace(/:/g, '\\:')
                    .replace(/,/g, '\\,')
                    .replace(/'/g, "\\'");
    ...
    repairAndEncodeForVav(inputJSON); 
  ```
...
- The file attached contains:
  - VAV_Native_Agent_Autonomous_DynamicJsonPayload -  Virtual Agent Voice with Fallback as well as agent transfer and end session.
- Call studio application is present in folder named "VAV_Native_Agent_Autonomous_DynamicJsonPayload_CS" which can be imported and used
- Deployed application on vxml server is present in folder named "VAV_Native_Agent_Autonomous_DynamicJsonPayload-deployed"
